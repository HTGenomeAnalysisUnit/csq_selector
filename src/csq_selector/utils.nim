...
proc cleanTxVersion*(x: string, regexp: Regex): string {.inline.} =
    result = x.replace(regexp)

proc cleanTxVersion*(x: seq[string], regexp: Regex): seq[string] {.inline.} =
    for tx in x: result.add(tx.cleanTxVersion(regexp))
